...
import type { LiqeQuery, InternalHighlight, InternalTest } from "./types";
...
const createValueTest = (ast: LiqeQuery): InternalTest => {
  ...
  if (ast.operator && ast.operator.operator !== ":") {
    ...
    if (typeof expressionValue !== "number") {
      // Allow non-numeric comparison if the operator is `=`.
      // This is used to strictly check for equality.
